<!DOCTYPE html>
<html class="html">
 <head>

  <meta http-equiv="Content-type" content="text/html;charset=UTF-8"/>
  <meta name="generator" content="7.0.314.244"/>
  <title>Servicios_empresariales</title>
  <!-- CSS -->
    		<link rel="shortcut icon" type="x-icon" href="images/icon.png" /><!--para logo en barra-->
    		
  <link rel="stylesheet" type="text/css" href="css/site_global.css?417434784"/>
  <link rel="stylesheet" type="text/css" href="css/master_b-master.css?471741950"/>
  <link rel="stylesheet" type="text/css" href="css/servicios_empresariales.css?3821850208" id="pagesheet"/>
  <!-- Other scripts -->
  <script type="text/javascript">
   document.documentElement.className += ' js';
</script>
   </head>
 <body>

  <div class="clearfix" id="page"><!-- column -->
   <div class="position_content" id="page_position_content">
    <div class="clearfix colelem" id="pu500"><!-- group -->
     <div class="browser_width grpelem" id="u500"><!-- group -->
      <div class="clearfix" id="u500_align_to_page">
       <a class="nonblock nontext clip_frame grpelem" id="u510" href="index.php"><!-- image --><img class="block" id="u510_img" src="images/logo-endoperio-dental-center.jpg" alt="" width="134" height="38"/></a>
      </div>
     </div>
     <div class="grpelem" id="u501"><!-- state-based BG images --></div>
     <a class="nonblock nontext grpelem" id="u502" href="index.php"><!-- state-based BG images --><img id="u502_states" alt="Inicio" src="images/blank.gif"/></a>
     <div class="grpelem" id="u503"><!-- state-based BG images --></div>
     <a class="nonblock nontext grpelem" id="u504" href="clinica.php"><!-- state-based BG images --><img id="u504_states" alt="Clinica" src="images/blank.gif"/></a>
     <div class="grpelem" id="u505"><!-- state-based BG images --></div>
     <a class="nonblock nontext grpelem" id="u506" href="servicios.php"><!-- state-based BG images --><img id="u506_states" alt="Servicios" src="images/blank.gif"/></a>
     <div class="grpelem" id="u507"><!-- state-based BG images --></div>
     <a class="nonblock nontext grpelem" id="u508" href="pacientes.php"><!-- state-based BG images --><img id="u508_states" alt="Pacientes" src="images/blank.gif"/></a>
    </div>
    <img class="colelem" id="u738-4" src="images/u738-4.png" alt="Servicios Empresariales" width="221" height="26"/><!-- rasterized frame -->
    <img class="colelem" id="u737-17" src="images/u737-17.png" alt="En la actualidad, la cultura laboral nos ha llevado a encontrar mecanismos para incrementar la productividad de su empresa. En los rubros de salud sabemos que existen ciertos padecimientos o enfermedades que nos afectarán directamente en el desempeño de sus trabajadores y por consecuencia en la eficiencia. Se tiene bien reconocido que las enfermedades de origen bucodental son padecimientos que generalmente son dolorosos y que se manifiestan en la noche o a cualquier hora, y que provocan ausentismo, somnolencia, estrés y en general un malestar que nos impedirá un adecuado desempeño de las actividades de su personal, llegando a provocar hasta el 7% de disminución en la productividad en un periodo de un año. Esto no solamente afecta al individuo, también a cualquier integrante de la familia que se vea en esta situación lo afectará directamente. Es bien sabido que la boca juega un papel importante en el desarrollo de relaciones interpersonales del individuo, una boca sana y con una apariencia agradable nos proporciona muy buena presentación y seguridad para relacionarnos. Los dientes manchados o pigmentados, el mal aliento (halitosis) y la malposición dentaria son los causales más frecuentes de incomodidad en los pacientes.
Es por eso que en ENDOPERIO DENTAL CENTER conocemos perfectamente esta situación y ponemos a su disposición Servicios Odontológicos Especializados para usted y su personal, procurando contribuir desde nuestras posibilidades incrementar la productividad eficiencia de su empresa. Para este objetivo le presentamos las siguientes opciones: " width="862" height="306"/><!-- rasterized frame -->
    <div class="clearfix colelem" id="pu772-7"><!-- group -->
     <img class="grpelem" id="u772-7" src="images/u772-7.png" alt="&nbsp;A.&#45; Esquema de Cobertura Total: " width="221" height="76"/><!-- rasterized frame -->
     <img class="grpelem" id="u774-6" src="images/u774-6.png" alt="
B.&#45;Esquema Porcentual de Cobertura:
" width="221" height="76"/><!-- rasterized frame -->
     <img class="grpelem" id="u775-6" src="images/u775-6.png" alt="
C.&#45;Esquema de Atención Individual:
" width="221" height="76"/><!-- rasterized frame -->
    </div>
    <div class="clearfix colelem" id="pu773-4"><!-- group -->
     <img class="grpelem" id="u773-4" src="images/u773-4.png" alt="En este sistema la empresa cubre el 100% de los honorarios de la atención de sus empleados, obteniendo así, comprobantes fiscales totalmente deducibles de impuestos. Directamente usted o su departamento de recursos humanos nos proporciona los nombres de los empleados que obtengan este beneficio." width="200" height="178"/><!-- rasterized frame -->
     <div class="clearfix grpelem" id="u776-3"><!-- content -->
      <p>&nbsp;</p>
     </div>
     <img class="grpelem" id="u777-5" src="images/u777-5.png" alt="En este plan el empleado cubrirá un porcentaje de los honorarios de la atención odontológica y la cantidad restante será cubierta por la empresa, dicho porcentaje será establecido previamente y en el cual también se expedirá el respectivo comprobante fiscal.
" width="200" height="178"/><!-- rasterized frame -->
     <img class="grpelem" id="u779-5" src="images/u779-5.png" alt="En este servicio el paciente cubrirá en su totalidad el costo de sus tratamientos odontológicos, sin embargo obtendrá los beneficios de tarifas preferenciales y la atención de nuestra clínica.
" width="200" height="178"/><!-- rasterized frame -->
    </div>
    <img class="colelem" id="u780-5" src="images/u780-5.png" alt="De cualquier manera para cualquier duda o información complementaria estamos a sus servicios
" width="674" height="34"/><!-- rasterized frame -->
    <div class="verticalspacer"></div>
    <div class="browser_width colelem" id="u1098"><!-- simple frame --></div>
    <div class="clearfix colelem" id="pu1105"><!-- group -->
     <div class="clip_frame grpelem" id="u1105"><!-- image -->
      <img class="block" id="u1105_img" src="images/en5.png" alt="" width="32" height="29"/>
     </div>
     <img class="grpelem" id="u1099-7" src="images/u1099-7.png" alt="Dr. Alejandro González Merlo
CIRUJANO DENTISTA ENDOPERIODONCISTA
" width="268" height="51"/><!-- rasterized frame -->
    </div>
    <!--div class="clearfix colelem" id="ppu1112"><!-- group --
     <div class="clearfix grpelem" id="pu1112"><!-- column --
      <div class="clip_frame colelem" id="u1112"><!-- image --
       <img class="block" id="u1112_img" src="images/direccion.png" alt="" width="32" height="32"/>
      </div>
      <div class="clip_frame colelem" id="u1110"><!-- image --
       <img class="block" id="u1110_img" src="images/phone.png" alt="" width="35" height="35"/>
      </div>
      <div class="clip_frame colelem" id="u1108"><!-- image --
       <img class="block" id="u1108_img" src="images/email.png" alt="" width="35" height="35"/>
      </div>
     </div>
     <div class="clearfix grpelem" id="pu1102-6"><!-- column --
      <img class="colelem" id="u1102-6" src="images/u1102-6.png" alt="Planta El Infiernillo No. 101 , Col. Electricistas. Morelia Michoacán. México." width="321" height="34"/>
      <!-- rasterized frame --
      <div class="clearfix colelem" id="pu1107-6"><!-- group --
       <img class="grpelem" id="u1107-6" src="images/u1107-6.png" alt="CITAS
+52 443 324 8008" width="138" height="37"/><!-- rasterized frame --
       <img class="grpelem" id="u1100-6" src="images/u1100-6.png" alt="URGENCIAS
044 443 114 4047" width="152" height="40"/><!-- rasterized frame --
      </div>
      <img class="colelem" id="u1101-4" src="images/u1101-4.png" alt="contacto@endoperio.com.mx" width="200" height="17"/>
      <!-- rasterized frame --
     </div>
     <div class="grpelem" id="u1115"><!-- simple frame --</div
     <div class="clearfix grpelem" id="u1114-4"><!-- content --
      <p>sesion</p>
     </div>
    </div>
    <a class="nonblock nontext clip_frame colelem" id="u1103" href="http://www.webox.org.mx">
      <!-- image --<img class="block" id="u1103_img" src="https://cdn4.iconfinder.com/data/icons/iconsimple-logotypes/512/github-32.png"alt="" width="89" height="28"/></a>
   </div>
  </div>
  <div class="preload_images">
   <img class="preload" src="images/home1h-u501-r.png" alt=""/>
   <img class="preload" src="images/u502_states-r.png" alt=""/>
   <img class="preload" src="images/u502_states-a.png" alt=""/>
   <img class="preload" src="images/homeh-u503-r.png" alt=""/>
   <img class="preload" src="images/u504_states-r.png" alt=""/>
   <img class="preload" src="images/u504_states-a.png" alt=""/>
   <img class="preload" src="images/en2h-u505-r.png" alt=""/>
   <img class="preload" src="images/u506_states-r.png" alt=""/>
   <img class="preload" src="images/u506_states-a.png" alt=""/>
   <img class="preload" src="images/en3h-u507-r.png" alt=""/>
   <img class="preload" src="images/u508_states-r.png" alt=""/>
   <img class="preload" src="images/u508_states-a.png" alt=""/>
  </div>
  <!-- JS includes -->
  <script type="text/javascript">
   if (document.location.protocol != 'https:') document.write('\x3Cscript src="http://musecdn.businesscatalyst.com/scripts/4.0/jquery-1.8.3.min.js" type="text/javascript">\x3C/script>');
</script>
  <script type="text/javascript">
   window.jQuery || document.write('\x3Cscript src="scripts/jquery-1.8.3.min.js" type="text/javascript">\x3C/script>');
</script>
  <script src="scripts/museutils.js?3865766194" type="text/javascript"></script>
  <script src="scripts/jquery.tobrowserwidth.js?3842421675" type="text/javascript"></script>
  <script src="scripts/jquery.watch.js?4068933136" type="text/javascript"></script>
  <!-- Other scripts -->
  <script type="text/javascript">
   $(document).ready(function() { try {
Muse.Utils.transformMarkupToFixBrowserProblemsPreInit();/* body */
$('.browser_width').toBrowserWidth();/* browser width elements */
Muse.Utils.prepHyperlinks(true);/* body */
Muse.Utils.fullPage('#page');/* 100% height page */
Muse.Utils.showWidgetsWhenReady();/* body */
Muse.Utils.transformMarkupToFixBrowserProblems();/* body */
} catch(e) { Muse.Assert.fail('Error calling selector function:' + e); }});
</script>
   </body>
</html>