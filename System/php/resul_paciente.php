...
while ($row2 = $result2->fetch_row())) {
	...
	$result = $conn->query("SELECT * from Fotografias_Clinicas where id_paciente='$buscar' order by fecha_foto desc;");
	echo "<table>";
	//while ($row_clinica = mysql_fetch_array($result3, MYSQL_NUM)) {
	while ($row_clinica = $result->fetch_row())) {
	...
	//$result = mysql_query("select * from Fotografias_Externas where id_paciente='$buscar' order by fecha_foto desc;;");
	$result = $conn->query("SELECT * from Fotografias_Externas where id_paciente='$buscar' order by fecha_foto desc;;");
	echo "<table>";
	//while ($row_externa = mysql_fetch_array($result, MYSQL_NUM)) {
	while ($row_externa = $result->fetch_row())) {
	...
	//$result = mysql_query("select * from Radiografias where id_paciente='$buscar' order by fecha_foto desc;;");
	$result = $conn->query("SELECT  * from Radiografias where id_paciente='$buscar' order by fecha_foto desc;;");
	echo "<table>";
	//while ($row_radio = mysql_fetch_array($result, MYSQL_NUM)) {
	while ($row_radio = $result->fetch_row())) {
	...
	//$result = mysql_query("select * from Recetas where id_paciente='$buscar' order by fecha desc limit 10;");
	$result = $conn->query("SELECT  * from Recetas where id_paciente='$buscar' order by fecha desc limit 10;");
