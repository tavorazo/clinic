...
if($S == 1){
    ...
    echo "<a href='?semana_b=",$semana_siguiente,"&ano=",$ano_s,"&S=1'>
        ...
        if($semana_b=='')
		$result2 = mysql_query("select * from recibos where fecha like '%$fechab%'");
	else
...
echo "<table border=1 style='margin-top:100px; '>
	<tr>
          ...
          <td style='color:#58ACFA'>Comprador 		</td>
          <td style='color:#58ACFA'>Fecha		</td>
          <td style='color:#58ACFA'>Descripcion 			</td>
          <td style='color:#58ACFA'>Total Pagado			</td>
      ...
      echo '<td><a href="../php/imprimir_recibo.php?id_recibo=',$deuda,'" >Imprimir</a></td></tr>';
      } 
      ...
      $deuda = $row2[0];
      $ano = date(Y);
      /*$result3 = mysql_query("select * from pagos_historia where semana='semana_b' && id_adeudo='$deuda' && y='$ano' limit 1");
      
      print "select * from pagos_historia where semana='$semana_b' && id_adeudo='$deuda' && y='$ano' limit 1";
