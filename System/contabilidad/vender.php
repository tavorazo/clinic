...
<form action="generar_venta.php" method="post" style="margin:200px 0px;">
  <table style="width:900px;">
    ...
    <td>
     ...
     // include("../php/base.php");
      ...
      $sql = "SELECT * FROM  inventario where venta='1'";
      $result1 = $conn->query($sql);
      echo "<center><select name='producto' class='campoT' style='width:150px; margin:auto auto'>";
      setlocale(LC_MONETARY, 'en_US');
      //while ($row2 = mysql_fetch_array($result2, MYSQL_NUM)){
      while ($concept = $result1->fetch_assoc()) {
        echo "<option value='",$concept['id_producto'],"'>",$concept['nombre']," ",$concept["numero_serial"],". Precio : " ,money_format('%(#10n',$concept['precio_venta']),"</option>";
      }
      echo "</select></center>";
      ?>
    </td><td>
  ...
  <select name="tipo" class="campoT" style="width:190px; margin:auto auto; " id="options"  onchange="optionCheck()">
    ...
    <option value="0">No es paciente</option>
            ...
            $sql = "SELECT id_paciente, nombres, apellido_paterno, apellido_materno from paciente";
            $result2 = $conn->query($sql);
            echo "<center><select name='id_paciente' class='campoT' style='visibility:hidden; margin:auto auto' id='si_cliente'>";
            echo "<option></option>";
           // while ($paciente = mysql_fetch_array($result2, MYSQL_NUM)){
            while ($paciente = $result2->fetch_assoc()) {
              echo "<option value='",$paciente['id_paciente'],"'>" ,$paciente['id_paciente'],"-",$paciente['nombres']," ",$paciente['apellido_paterno']," ",$paciente['apellido_materno'],"</option>";
            }
            echo "</select></center>";
            ?>
            <br>
          ...
          <!--<input type="hidden" value="<?php echo $id_paciente; ?>" name="id_paciente" style="width:90px; margin:auto auto">-->
