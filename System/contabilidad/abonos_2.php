...
if($semana_b=='')
	$result_pagos = $conn->query("SELECT * from pagos_historia where id_tipo='3' and id_adeudo='$deuda' and  fecha like '%$fecha%'");
	//$result_pagos = $conn->query("SELECT * from pagos_historia where id_tipo='3' and id_adeudo='$deuda' and  fecha like '%$fecha%'");
else
	$result_pagos = $conn->query("SELECT * from pagos_historia where id_tipo='3' and  id_adeudo='$deuda' and  semana='$semana_b' and y='$y'");
	//$result_pagos = $conn->query("SELECT * from pagos_historia where id_tipo='3' and  id_adeudo='$deuda' and  semana='$semana_b' and y='$y'");
